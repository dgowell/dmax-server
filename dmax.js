 ...
 */
function selectRecentClients(maxtime, limit, callback) {
    MDS.sql("SELECT * FROM CLIENTS WHERE recdate<" + maxtime + " ORDER BY recdate DESC LIMIT " + limit, function (sqlmsg) {
        callback(sqlmsg);
    });
}
...
/**
 * Send message via Maxima to publickey
 */
function sendMaximaMessage(message, publickey, callback) {
    var maxcmd = "maxima action:send poll:true publickey:" + publickey + " application:chatter data:" + JSON.stringify(message);
    MDS.cmd(maxcmd, function (msg) {
        MDS.log(JSON.stringify(msg));
        if (callback) {
            callback(msg);
